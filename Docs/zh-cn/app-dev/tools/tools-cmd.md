...
```bash
keels <subcommand> [options]
```

|参数|功能|
|---|---|
|`-h`，`--help` | 显示全局帮助信息。可使用 `keels <subcommand> --help` 查看子命令帮助信息。|
|`-V`，`--version` | 显示 `keels` 版本号。|

### doctor 命令

用于检查当前环境配置。当前会检查 Android、HarmonyOS、VS Code、网络环境；在 macOS 平台下，还会检查 iOS/Xcode 相关环境。
...
**注意：** 当前支持检测 Android、HarmonyOS 和 iOS 设备；在 macOS 平台下，还支持检测已启动的 iOS 模拟器。设备平台标识包括：`android-arm64`、`ohos-arm64`、`ios-arm64`、`ios-sim-arm64`。
...
|`--machine` | 输出 JSON 格式的设备信息，便于 IDE 或脚本集成。|
...
|`--module` | 等价于 `-t module`。|
|`--app-type <keels, native>` | 用于在 `logic-module` 模式下指定壳工程的类型。`keels` 表示壳工程使用 Keels UI 框架构建，`native` 表示使用各平台原生语言和 UI 框架构建，默认值为 `keels`。|
|`-n <name>`，`--name <name>` | 工程名称，会影响 bundleName 和模板中的包名。|
|`--org` | 指定组织名，默认值为 `com.example`。|
|`--android-toolchain <modern, legacy>` | 指定 Android 构建工具链，默认值为 `modern`；`legacy` 用于兼容 JDK8 的旧版 Android 工具链。|
|`--ios-export-method <debugging, release-testing, app-store, enterprise>` | 创建 `app` 工程时指定 iOS 导出方式，默认值为 `debugging`。|
...
1. HarmonyOS 工程在执行 `run` 命令时支持 `--autosign` 自动化签名，但不用于实际版本发布，仅提高开发测试效率。亦可以使用 DevEco Studio 进行手动签名，手动签名的详细流程见[附录A：HarmonyOS 应用签名流程介绍](tools-cmd.md#附录aharmonyos-应用签名流程介绍)。
2. iOS 工程首次构建需要在 Xcode 中设置自动签名。详细流程见 [附录B：iOS 应用签名流程介绍](#附录bios-应用签名流程介绍)。

|参数|功能|
|---|---|
|`-h`，`--help` | 显示命令的帮助信息，不执行命令本身。|
|`-v`，`--verbose` | 显示执行过程中的详细信息。|
|`package`| `app` 工程支持 `apk`、`hap`；在 macOS 平台下，还支持 `ios-sim`、`ios` 和 `ipa`。`logic-module` 和 `module` 工程支持 `aar`、`har`；在 macOS 平台下，还支持 `framework-sim` 和 `framework`。|
|`-d <name or id>`，`--device-id <name or id>` | 指定目标终端。`-d`和`--platform`只能指定一个。如果只有一个目标平台，默认为该平台；如果有多个需要指定，使用`--platform`指定默认平台。|
|`--platform <platform>` | 指定编译的目标平台。不同 `package` 支持的平台不同：`apk`/`aar` 对应 `android-arm64`，`hap`/`har` 对应 `ohos-arm64`，`ios`/`ipa`/`framework` 对应 `ios-arm64`，`ios-sim`/`framework-sim` 对应 `ios-sim-arm64`。|
...
|`-h`，`--help` | 显示命令的帮助信息，不执行命令本身。|
|`-v`，`--verbose` | 显示执行过程中的详细信息。|
|`-d <name or id>`，`--device-id <name or id>` | 指定运行的目标终端。如果只有一个可用设备，默认使用该设备；如果存在多个可用设备，则需要显式指定。|
...
|`--cold`| 与 `--launch` 一起使用，用于冷启动模拟器，仅适用于 Android 模拟器。|

## devtools 基础命令介绍
### 启动录制
```bash
devtools cpu start --platform <平台> --app|pid <应用名|进程ID> [--output <目标路径> --device <设备序列号>]
```
...
|参数|必填|说明|默认值|
|---|---|---|---|
|`--platform` | 是 | 指定设备平台，当前支持：ohos, android 和 ios | 无 |
|`--app\|pid` | 是 | 目标应用名 \| 目标进程ID | 无 |
|`--output` | 否 | 输出文件存储路径 | 当前工作目录 |
|`--device` | 否 | 指定待调优进程所在的设备，只有一台设备连接时不需要填写 | 如果只有一台设备则默认为此设备 |
