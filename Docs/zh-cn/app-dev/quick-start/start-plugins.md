...
- 在 VS Code 中右击 **CJMP-x.x.x.vsix** 和 **cangjie-x.x.x.vsix** 文件，在下拉列表中选择 **安装插件 VSIX（Install Extension VSIX）** 安装插件。
...
1. 点击页面左侧的 **插件** 按键，在扩展中搜索 `CJMP`和`cangjie`，可以看到已经安装的 `CJMP` 和 `cangjie` 插件。
...
3. 配置 CJMP SDK Path：  
    ...
    - **方法1：**  
    打开 VSCode 设置页面，搜索 `CJMP`，在搜索结果中选择 `扩展`->`CJMP Configuration`，将 SDK 中的 `bin` 路径（例如：Path\to\cjmp-sdk\cjmp-tools\bin）添加到配置项。
    ![sdk-path-setting](image/sdk-path-setting.png)

    - **方法2：**  
    VSCode 设置中 CJMP SDK 路径未配置或者配置错误时，点击 `CJMP: Create CJMP Project` 命令，右下方会弹出提醒需要配置 SDK Path，点击 OK，选择 SDK 目录（例如：Path\to\cjmp-sdk）。

4. 配置 CJ Native Backend：
    打开 VSCode 设置页面，搜索 `cangjie`，在搜索结果中选择 `扩展`->`Cangjie`->`Cangjie Language Support`，将 SDK 中的 `cangjie-android` 路径（例如：Path\to\cjmp-sdk\cjmp-tools\third_party\cangjie-android）添加到配置项。
...
运行成功后会将应用推送至终端并打开初始页面。应用成功推送到终端设备后，当调试控制台开始输出日志时，即表示可以开始调试。
