...
- macos:
```shell
bash build.sh -p systemlibs -f macos -m debug [-o android]
```
> 暂不支持linux构建
> 不支持--mode设定为profile
...
- Android 编译:
```shell
bash build.sh -p cj-ui -o android
```

- iOS 编译:
```shell
bash build.sh -p cj-ui -o ios
bash build.sh -p cj-ui -o ios-sim   # iOS 模拟器产物编译
```