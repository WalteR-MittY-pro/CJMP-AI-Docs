...
```cangjie
import cjmp.ability.ui_ability.*
```
...
| 参数名  | 类型      | 必填 | 默认值 | 说明                |
| :------ | :-------- | :--- | :----- | :------------------ |
| context | JSContext | 是   | -      | ArkTS互操作上下文。 |
| input   | JSValue   | 是   | -      | ArkTS统一类型。     |
...
**功能：** 从JSValue转换为[ApplicationContext](#class-applicationcontext)类型。该转换仅可在函数传递中使用。
...
**功能：** 从JSValue转换为[Context](./cj-apis-app-ability-ui_ability.md#class-context)类型。该转换仅可在函数传递中使用。
...
**功能：** 从JSValue转换为[UIAbilityContext](#class-uiabilitycontext)类型。
...
| 参数名      | 类型        | 必填 | 默认值 | 说明                                                           |
| :---------- | :---------- | :--- | :----- | :------------------------------------------------------------- |
| want        | Want        | 是   | -      | 当前UIAbility的Want类型信息，包括UIAbility名称、Bundle名称等。 |
| launchParam | LaunchParam | 是   | -      | 创建 ability、上次异常退出的原因信息。                         |
...
| launchParam | LaunchParam | 是   | -      | UIAbility启动参数，包含启动原因等。     |
...
**功能：** 当WindowStage实例创建完成后，系统会触发该回调。开发者可以在该回调中通过WindowStage加载页面。
