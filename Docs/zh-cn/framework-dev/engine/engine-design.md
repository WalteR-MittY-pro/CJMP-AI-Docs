...
多UI前端适配层提供两套API接口，可分别支持Taro\React Native\CJ-UI等。

- CJ-UI

以Text为例，一个简单的显示Hello Cangjie文本的应用如下（备注：CJ-UI依赖仓颉SDK，请先申请[仓颉语言开发者预览版](https://developer.huawei.com/consumer/cn/activityDetail/cangjie-beta/)）：
...
````
其中，以@Entry修饰的自定义组件MyView作为CJ-UI界面入口，build函数为入口函数。
