...
private boolean validatePath(String path) {
    try {
        String canonicalHomePath = new File(HOME_PATH).getCanonicalPath();
        String expectedPrefix = canonicalHomePath.endsWith(File.separator)
            ? canonicalHomePath : canonicalHomePath + File.separator;
        if (path.equals(canonicalHomePath) || path.startsWith(expectedPrefix)) {
            return true;
        }
    } catch (IOException ex) {
        return false;
...
private String sanitizeFileName(String fileName, String dir) throws IOException {
    ...
    String expectedPrefix = canonicalDir.endsWith(File.separator)
        ? canonicalDir : canonicalDir + File.separator;
    if (canonicalPath.equals(canonicalDir) || canonicalPath.startsWith(expectedPrefix)) {
        return canonicalPath;
    }
    throw new IOException("Path Traversal vulnerability: ...");
}

private void createDir(File dirPath) throws IOException {
...
实际应用中，通常采用三方件实现对json、xml、yaml格式的数据序列化和反序列化操作。常用的三方件包括：fastjson、jackson、XMLDecoder、XStream、SnakeYAML等。
...
try (ObjectInputStream ois = new ObjectInputStream(fis)) {
    PersonInfo myPerson = (PersonInfo) ois.readObject();
}
```

上面的示例中，当反序列化操作的对象是攻击者构造的DeserializeExample对象的序列化结果，当`PersonInfo myPerson = (PersonInfo) ois.readObject()`该语句执行时会报错，但是DeserializeExample对象中的`readObject()`方法中的攻击代码已经被执行。
