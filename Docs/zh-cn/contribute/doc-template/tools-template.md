...
使用本工具前，开发者需要安装hdc工具，安装步骤参考[开发准备](https://gitcode.com/CJMP/Docs/blob/main/zh-cn/app-dev/quick-start/start-overview.md) `HarmonyOS 端依赖工具` 章节，最后执行hdc shell。


## 命令列表
> *写作说明*
>1. 命令列表的获取，执行"xx -h"或"xx -help"。
>2. 将得到的返回值中每一个命令列举出来，放到表格中（help/-h 放到第一行），剩下的命令可以按照使用频率排列。
